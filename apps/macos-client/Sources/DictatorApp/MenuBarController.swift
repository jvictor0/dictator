...
final class MenuBarController {
    ...
    static func makeMenu(stateMenuItem: NSMenuItem, launchpadStatusMenuItem: NSMenuItem, target: AnyObject) -> NSMenu {
        ...
        let setKeyItem = NSMenuItem(title: "Set OpenAI Key (Memory)…", action: #selector(setOpenAIKey), keyEquivalent: "k")
        setKeyItem.target = target
        menu.addItem(setKeyItem)
        let clearKeyItem = NSMenuItem(title: "Clear OpenAI Key (Memory)", action: #selector(clearOpenAIKey), keyEquivalent: "")
